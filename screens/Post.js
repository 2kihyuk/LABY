...
//           </TouchableOpacity>
//           <View style={styles.iconSeparator} />
//           <Feather name="send" size={24} color="black" onPress={handleChatModal} />
//           <ChatModal isVisible={isChatModalVisible} onClose={handleChatModal} creatorName={userId} />
  ...
  return (
    <KeyboardAvoidingView behavior="padding" style={{ flex: 1 }}>
      <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 100 }}>
        <View style={styles.profile}>
          <Image
            source={creatorInfo.imageUrl ? { uri: creatorInfo.creatorImage } : require('../assets/profile.png')}
          ...
          {comments.map(comment => (
            <TouchableWithoutFeedback key={comment.id} onLongPress={() => handleDeleteComment(comment.id)}>
              <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 10 }}>
                <Image
                  style={styles.myComment}
                  source={{ uri: comment.commenter.profileImageUrl ? comment.commenter.profileImageUrl : 'https://i.namu.wiki/i/shlrpQDvlqdGXhFEhsY9VJOX6CU6A9ZxVv0TDAxLAUjhxoAkvQDBdfS6qhjUpQo5D94X1YPhMQRp-_F0GgQRHjK9hma6I_f2SHklZ7ZOa6OO9Oxtsg9JdGajbqImDawbQZj-H5fS9QFV3JM3I3BCRA.webp' }}
                  resizeMode={'stretch'}
                />
                <Text style={{ fontWeight: 'bold' }}>{comment.commenterId}  </Text><Text>{comment.content}{comment.text}</Text>
              </View>
